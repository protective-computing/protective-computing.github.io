Set-StrictMode -Version Latest
$ErrorActionPreference = 'Stop'

$baseUrl = 'https://protective-computing.github.io'

$entries = @(
  @{ loc = "$baseUrl/"; file = 'index.html'; changefreq = 'weekly'; priority = '1.0' },

  @{ loc = "$baseUrl/docs/audit-this-site.html"; file = 'docs/audit-this-site.html'; changefreq = 'monthly'; priority = '0.95' },
  @{ loc = "$baseUrl/docs/legitimacy.html"; file = 'docs/legitimacy.html'; changefreq = 'monthly'; priority = '0.94' },
  @{ loc = "$baseUrl/docs/standards-crosswalks.html"; file = 'docs/standards-crosswalks.html'; changefreq = 'monthly'; priority = '0.93' },
  @{ loc = "$baseUrl/docs/standards-crosswalk-annexes.html"; file = 'docs/standards-crosswalk-annexes.html'; changefreq = 'monthly'; priority = '0.92' },
  @{ loc = "$baseUrl/docs/release-history.html"; file = 'docs/release-history.html'; changefreq = 'monthly'; priority = '0.9' },
  @{ loc = "$baseUrl/docs/getting-started.html"; file = 'docs/getting-started.html'; changefreq = 'monthly'; priority = '0.9' },
  @{ loc = "$baseUrl/docs/external-review-packet-template.html"; file = 'docs/external-review-packet-template.html'; changefreq = 'monthly'; priority = '0.91' },
  @{ loc = "$baseUrl/docs/rfc-process.html"; file = 'docs/rfc-process.html'; changefreq = 'monthly'; priority = '0.91' },
  @{ loc = "$baseUrl/docs/what-protective-computing-is-not.html"; file = 'docs/what-protective-computing-is-not.html'; changefreq = 'monthly'; priority = '0.9' },
  @{ loc = "$baseUrl/docs/stability-assumption.html"; file = 'docs/stability-assumption.html'; changefreq = 'monthly'; priority = '0.9' },
  @{ loc = "$baseUrl/docs/offline-first-health-architecture.html"; file = 'docs/offline-first-health-architecture.html'; changefreq = 'monthly'; priority = '0.9' },
  @{ loc = "$baseUrl/docs/trauma-informed-software-patterns.html"; file = 'docs/trauma-informed-software-patterns.html'; changefreq = 'monthly'; priority = '0.9' },
  @{ loc = "$baseUrl/docs/audit-checklist.html"; file = 'docs/audit-checklist.html'; changefreq = 'monthly'; priority = '0.9' },
  @{ loc = "$baseUrl/docs/what-software-gets-wrong-about-instability.html"; file = 'docs/what-software-gets-wrong-about-instability.html'; changefreq = 'monthly'; priority = '0.9' },
  @{ loc = "$baseUrl/docs/designing-for-coercion-resistance.html"; file = 'docs/designing-for-coercion-resistance.html'; changefreq = 'monthly'; priority = '0.9' },
  @{ loc = "$baseUrl/docs/local-authority-vs-cloud-dependence.html"; file = 'docs/local-authority-vs-cloud-dependence.html'; changefreq = 'monthly'; priority = '0.9' },
  @{ loc = "$baseUrl/docs/degraded-mode-ux-patterns.html"; file = 'docs/degraded-mode-ux-patterns.html'; changefreq = 'monthly'; priority = '0.9' },
  @{ loc = "$baseUrl/docs/spec/v1.0.html"; file = 'docs/spec/v1.0.html'; changefreq = 'yearly'; priority = '0.95' },
  @{ loc = "$baseUrl/docs/spec/v1.0-must-justifications.html"; file = 'docs/spec/v1.0-must-justifications.html'; changefreq = 'monthly'; priority = '0.94' },
  @{ loc = "$baseUrl/docs/reference-implementation/paintracker-mapping.html"; file = 'docs/reference-implementation/paintracker-mapping.html'; changefreq = 'monthly'; priority = '0.92' },
  @{ loc = "$baseUrl/docs/reference-implementation/paintracker-reference-packet.html"; file = 'docs/reference-implementation/paintracker-reference-packet.html'; changefreq = 'monthly'; priority = '0.91' },
  @{ loc = "$baseUrl/docs/reviews/external-review-0001-paintracker-reference-packet.html"; file = 'docs/reviews/external-review-0001-paintracker-reference-packet.html'; changefreq = 'monthly'; priority = '0.9' },
  @{ loc = "$baseUrl/docs/independent-review.html"; file = 'docs/independent-review.html'; changefreq = 'monthly'; priority = '0.93' },
  @{ loc = "$baseUrl/docs/crosswalks/nist-privacy-framework.html"; file = 'docs/crosswalks/nist-privacy-framework.html'; changefreq = 'monthly'; priority = '0.89' },
  @{ loc = "$baseUrl/docs/crosswalks/nist-ai-rmf.html"; file = 'docs/crosswalks/nist-ai-rmf.html'; changefreq = 'monthly'; priority = '0.89' },
  @{ loc = "$baseUrl/docs/crosswalks/iso-27001.html"; file = 'docs/crosswalks/iso-27001.html'; changefreq = 'monthly'; priority = '0.89' },
  @{ loc = "$baseUrl/docs/crosswalks/iso-42001.html"; file = 'docs/crosswalks/iso-42001.html'; changefreq = 'monthly'; priority = '0.89' },
  @{ loc = "$baseUrl/docs/crosswalks/soc2.html"; file = 'docs/crosswalks/soc2.html'; changefreq = 'monthly'; priority = '0.89' },
  @{ loc = "$baseUrl/docs/crosswalks/owasp-asvs.html"; file = 'docs/crosswalks/owasp-asvs.html'; changefreq = 'monthly'; priority = '0.89' },

  # Directory URL resolves to docs/principles/index.html on GitHub Pages.
  @{ loc = "$baseUrl/docs/principles/"; file = 'docs/principles/index.html'; changefreq = 'monthly'; priority = '0.9' },

  @{ loc = "$baseUrl/docs/principles/reversibility.html"; file = 'docs/principles/reversibility.html'; changefreq = 'monthly'; priority = '0.85' },
  @{ loc = "$baseUrl/docs/principles/exposure-minimization.html"; file = 'docs/principles/exposure-minimization.html'; changefreq = 'monthly'; priority = '0.85' },
  @{ loc = "$baseUrl/docs/principles/local-authority.html"; file = 'docs/principles/local-authority.html'; changefreq = 'monthly'; priority = '0.85' },
  @{ loc = "$baseUrl/docs/principles/coercion-resistance.html"; file = 'docs/principles/coercion-resistance.html'; changefreq = 'monthly'; priority = '0.85' },
  @{ loc = "$baseUrl/docs/principles/degraded-functionality.html"; file = 'docs/principles/degraded-functionality.html'; changefreq = 'monthly'; priority = '0.85' },
  @{ loc = "$baseUrl/docs/principles/essential-utility.html"; file = 'docs/principles/essential-utility.html'; changefreq = 'monthly'; priority = '0.85' },

  # Important published artifact.
  @{ loc = "$baseUrl/PLS_RUBRIC_v1_0_rc1.pdf"; file = 'PLS_RUBRIC_v1_0_rc1.pdf'; changefreq = 'yearly'; priority = '0.6' }
)

function Get-LastMod([string] $path) {
  if (-not (Test-Path -LiteralPath $path)) {
    return $null
  }

  $value = (git log -1 --format=%cI -- $path 2>$null)
  if ($value) {
    return $value.Trim()
  }

  return (Get-Item -LiteralPath $path).LastWriteTimeUtc.ToString('yyyy-MM-ddTHH:mm:ssZ')
}

function XmlEscape([string] $value) {
  if ($null -eq $value) { return '' }
  return [System.Security.SecurityElement]::Escape($value)
}

$lines = New-Object System.Collections.Generic.List[string]
$lines.Add('<?xml version="1.0" encoding="UTF-8"?>')
$lines.Add('<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">')

foreach ($entry in $entries) {
  $lastMod = Get-LastMod -path $entry.file
  if (-not $lastMod) {
    # If a file is missing, skip it rather than emitting stale/incorrect metadata.
    continue
  }

  $lines.Add('  <url>')
  $lines.Add("    <loc>$(XmlEscape $entry.loc)</loc>")
  $lines.Add("    <lastmod>$(XmlEscape $lastMod)</lastmod>")
  $lines.Add("    <changefreq>$(XmlEscape $entry.changefreq)</changefreq>")
  $lines.Add("    <priority>$(XmlEscape $entry.priority)</priority>")
  $lines.Add('  </url>')
}

$lines.Add('</urlset>')

$outPath = Join-Path $PSScriptRoot '..\sitemap.xml'
Set-Content -LiteralPath $outPath -Value $lines -Encoding UTF8
Write-Host "Wrote sitemap: $outPath"
